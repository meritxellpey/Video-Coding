...
def cutter(N):
    ...
    ffmpeg_extract_subclip("Big_Buck_Bunny_1080_10s_1MB.mp4", start_time, end_time, targetname="cut_BBB.mp4")
...
def yuv_hist():
    ...
    subprocess.call(['ffplay', 'cut_BBB.mp4', '-vf', 'split=2[a][b],[b]histogram,format=yuva444p[hh],[a][hh]overlay'])


def resizer(size):
    sizing = "scale="+size
    os.system("ffmpeg -i cut_BBB.mp4 -vf "+sizing+" resized_BBB.mp4")


def audio_mono():
    os.system("ffmpeg -i BBB.mp4 -ac 2 stereo_BBB.mp4")
...
def main():
    ...
    N = float(input('Choose the number of seconds you want to cut the video by between 0 and 600: '))
    cutter(N)
    ...
    print("We will proceed by changing the audio from stereo to mono.")
    audio_mono()
